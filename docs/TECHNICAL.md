...
| 入（远端到本地） | Clipboard::do_handle_format_list_request | `0x8badb0` |

`on_clipboard_update` 监听本地剪贴板变化往远端推，关同步直接拦掉就不外推。

入向**不能**拦 `handle_clipboard_request`（`0x8bf810`）那个分发器。它是请求/应答 switch：收到对端的格式表/数据/文件消息后处理，并回发 Response（如 case 1 处理完无条件发 `ClipboardFormatListResponse`）。整块返回 0 等于收下请求却永不应答，对端（没补丁的原版 UU）延迟渲染握手一直等，把对端本地剪贴板卡在"已 EmptyClipboard + 挂着不兑现的渲染桩"状态——表现为对端剪贴板被清空且再也复制不了。

所以入向只钝化真正动本地剪贴板的落地点 `do_handle_format_list_request`（`0x8badb0`，里面是 `EmptyClipboard` + 循环 `SetClipboardData(fmt,0)` 装延迟渲染桩）。关同步时让它直接返回，分发器照常应答对端，握手正常完成，只是主控本地剪贴板不被对端镜像。配合拦 `on_clipboard_update`，双向都不同步且谁都不坏。
...
| Clipboard::do_handle_format_list_request | `0x8badb0` | 剪贴板入 |
